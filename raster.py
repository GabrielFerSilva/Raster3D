...
def render_pixel(context, ij):
    ...
    samples = context.num_samples * context.len_samples
    ...
    for _ in range(samples):
        ...
        if hit_rec.hit:
            ...
            pixel = pixel + shaded_color / samples
        else:
            # this is box filtering!
            pixel = pixel + context.scene.background / samples
    return (i, j, pixel)
...
def main(args, pool):
    ...
    context = Context(scene=scene, camera=camera, num_samples=args.num_samples, len_samples=args.len_samples)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-l', '--len_samples', type=int, help='Number of samples per pixel for lens camera', default=1)
