...
from src.materials import SimpleMaterial, SimpleMaterialWithShadows, TranslucidMaterial, CheckerboardMaterial, SpecularMaterial
...
class Scene(BaseScene):
    def __init__(self):
        ...
        self.max_depth = 5  # for reflections/refractions
        self.camera = Camera(
        ...
        mirror_material = SpecularMaterial(reflection_coefficient=1.0)
        ...
        self.add(
            Cube(center=Vector3D(-8, 0, 1.5), length=6), 
            mirror_material
        )
        
        self.add(
            Cube(center=Vector3D(0, 0, 1.5), length=0.7), 
            blue_material
        )

        self.add(
            Cube(center=Vector3D(15, 0, 1.5), length=6), 
            mirror_material
        )
