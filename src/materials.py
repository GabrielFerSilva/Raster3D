...
class SpecularMaterial(Material):
    ...
    def shade(self, hit_record, scene):
        ...
        #for checking the recursion limit
        if hit_record.ray.depth >= scene.max_depth:
           return Color(1, 0, 1)

        direction = hit_record.ray.direction
        normal = hit_record.normal

        # reflected ray based on the normal component of the direction ray
        reflection_dir = direction - normal * 2 * direction.dot(normal)


        #create the new ray and check if it hits something in the scene
        new_ray = Ray(hit_record.point, reflection_dir, hit_record.ray.depth + 1)
        reflection_hit = scene.hit(new_ray)


        if reflection_hit.hit:
            
            #if it hits something, we need to shade based on the material of the hit
            reflection_material = reflection_hit.material
            reflected_color = reflection_material.shade(reflection_hit, scene)
        else:
            #if it doesn't hit, its because we have a ray going to the infinte
            # then we have a ray going to the backgound
